...
This version was customized slightly by E. Hazen (hazen@bu.edu)
for use in a USB POV project (see http://edf.bu.edu/pov ).
Customization described at end of this document.
...
CUSTOMIZATION FOR USB POV
=========================
I modified this code slightly for use in a USB POV (persistence of vision)
device.  I wanted the boot loader to be automatically invoked when the
device powered when connected to a computer.  I modified slightly main.c
so that it provides a roughly 2 second start-up delay, and if any USB
traffic is detected during this time the boot loader starts.  If there
is no traffic the application program is started.
