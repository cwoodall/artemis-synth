...
/*
 * customized slightly for use on USB POV device by E. Hazen (hazen@bu.edu)
 * see http://edf.bu.edu/pov
 *
 * AUTO_DETECT_USB enables a mode where the bootloader is automatically
 * invoked if there is any USB activity with ~ 2 sec of power-up.
 * Otherwise the application program is launched.
 */
...
#ifdef AUTO_DETECT_USB
static unsigned long int countIdleUSBLoops;
uchar wasUSBActivity;
/* at 12Mhz, about 200k counts/second */
#define AUTO_USB_TIMEOUT 300000
#endif
...
#ifdef AUTO_DETECT_USB
      if( !wasUSBActivity)
	if(++countIdleUSBLoops > AUTO_USB_TIMEOUT)
	  leaveBootloader();
#endif
      usbPoll();
#if BOOTLOADER_CAN_EXIT
